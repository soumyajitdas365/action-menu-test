...
import 'package:flutter_ios_action_menu/providers.dart';
...
void main() {
  runApp(MultiProvider(providers: [
    ChangeNotifierProvider<ActionButtonProvider>.value(
        value: actionButtonProvider)
  ], child: IosActionMenuApp()));
}
...
class ActionMenu extends StatefulWidget {
  const ActionMenu({super.key, required this.child});
  final Widget child;
  @override
...
class ActionMenuListItem extends StatefulWidget {
  ...
  @override
  State<ActionMenuListItem> createState() => _ActionMenuListItemState();
}

class _ActionMenuListItemState extends State<ActionMenuListItem> {
  GlobalKey action_menu_item_key = GlobalKey();
  @override
  Widget build(BuildContext context) {
    ActionButtonProvider actionButtonProvider =
        Provider.of<ActionButtonProvider>(context);
    final keyContext = action_menu_item_key.currentContext;
    final box = keyContext?.findRenderObject() as RenderBox?;
    final pos = box?.localToGlobal(Offset.zero);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      actionButtonProvider.itemHeight = box?.size.height ?? 0;
    });
    return AnimatedScale(
      ...
      child: Container(
        height: actionButtonProvider.itemHeight,
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                image: DecorationImage(
                  image: AssetImage(
                      'assets/images/avatar-${widget.index + 1}.png'),
                  fit: BoxFit.cover,
                ),
              ),
            ),
